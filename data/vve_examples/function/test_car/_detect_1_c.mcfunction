#vve_examples:test_car/_detect_1_c
# 使用尺寸为1的盒子探测cube实例并进行介质探测
# 输出介质响应(各模块的临时对象)
# 需要传入世界实体为执行者
...
execute at @s positioned ~-0.5 ~-0.5 ~-0.5 as @e[dx=0,dy=0,dz=0,tag=vve_cube_box,tag=vve_material_box] run function vve_examples:test_car/detect_point_c