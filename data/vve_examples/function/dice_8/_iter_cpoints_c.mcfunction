#vve_examples:dice_8/_iter_cpoints_c
# 遍历顶点作为碰撞点，进行介质探测
# 输出介质响应(各模块的临时对象)
...
execute store result score sstemp_ix int store result score sstemp_iy int store result score sstemp_iz int \
	store result score sstemp_kx int store result score sstemp_ky int store result score sstemp_kz int \
	run scoreboard players operation sstemp_jz int = a int
scoreboard players operation sstemp_jz int *= 3363 int
execute store result score sstemp_jx int store result score sstemp_jy int run scoreboard players operation sstemp_jz int /= 2378 int
