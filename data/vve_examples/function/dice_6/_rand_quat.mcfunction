#vve_examples:dice_6/_rand_quat
# 生成骰子随机四元数姿态
# 需要传入世界实体为执行者
...
execute store result score loop int run random value 0..5
execute if score loop int matches 1.. run function vve_examples:dice_6/rand_quat_loop
data modify entity 0-0-0-0-0 Pos set from storage vve_examples:io result[0]
...
scoreboard players operation quat_phi int /= 90 int
scoreboard players operation quat_phi int *= 900000 int
